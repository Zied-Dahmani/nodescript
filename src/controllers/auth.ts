...
export const login = async (req: express.Request, res: express.Response) => {
  try {
    ...
    const pwd = shaPassword(user.authentication.salt, password);
    
    if (user.authentication.password != pwd) {
      return res.sendStatus(403);
    }

    const salt = random();
    user.authentication.sessionToken = shaPassword(salt, user._id.toString());
